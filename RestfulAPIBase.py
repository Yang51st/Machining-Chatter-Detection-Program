...
class RestfulInterface:
  ...
  def Spindle(self):
    try:
      result=requests.put(self.urlString+"/DataService/Double/SID_RT_SPINDLE_TOOL_RPM",headers=self.headers,data='{ "data": 150.0 }',verify=False)
      logger.debug("Spindle Tool RPM is set to %s",result)
      if result.status_code!=HTTPStatus.OK:
        raise RestAPIException("Failed to change spindle speed.")
      return True
    except RestAPIException as exc:
      logger.debug("RestAPIException error: %s",exc.GetErrorMsg())
      return False 
